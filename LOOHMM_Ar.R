...
for (k in 1:Nrep)
{
  ...
  # ajuste
  fit <- stan(file = 'hmmAr.stan',data = data,chain=2,cores=4)
  ...
  for (j in 1:nsamples)
  {
    ...
    mod=list()
    mod$m=4
    mod$alpha=outs$alphas[samp[j],,]
    mod$betas=outs$betas1[samp[j],,]
    ...
    for (jj in 1:4)  
    {
      allprobs[1,jj]= dmvnorm(x[1,],mean=mod$alpha[,jj],sigma=diag((mod$sigmas[,jj])^2))  
      for (i in 2:(dim(x)[1]))    
      {
        mm=mod$alpha[,jj]+mod$betas[,jj]*x[i-1,]
        allprobs[i,jj]= dmvnorm(x[i,],mean=mm,sigma=diag((mod$sigmas[,jj])^2))  
      }   
    }
